...
void loop() {
  ...
  if (currentMillis - lastMillis >= 2000) {
    ...
    // Read discharging data (INA219 0x40)
    float shunt_load_mV = ina219_load.getShuntVoltage_mV();
    float bus_load_V = ina219_load.getBusVoltage_V();
    load_current_mA = readAverageCurrent(ina219_load, 10); // Average for noise reduction
    ...
    charge_current_mA = readAverageCurrent(ina219_charge, 10);
    float power_charge_mW = ina219_charge.getPower_mW();

    // Calculate battery voltage
    float internal_resistance = 0.2; // Adjust based on your battery
    battery_V = bus_load_V + (shunt_load_mV / 1000.0) + (load_current_mA / 1000.0 * internal_resistance);

    // Mode detection
    bool isDischarging = (load_current_mA > 1.0);
    bool isCharging = (!isDischarging && charge_current_mA > 0.2);
    String mode = isCharging ? "Charging" : isDischarging ? "Discharging" : "Idle";

    if (isCharging) {
      battery_V = bus_charge_V; 
    }
    // Alert detection
...
// Add this function for averaging current readings
float readAverageCurrent(Adafruit_INA219 &ina, int samples) {
  float sum = 0;
