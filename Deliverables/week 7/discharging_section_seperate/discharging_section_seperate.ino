...
void setup() {
  ...
  if (!ina219_load.begin()) {
    Serial.println("INA219 (0x40) failed. Check wiring: SDA->GPIO21, SCL->GPIO22, VCC->3V3, GND->GND, A0->GND, A1->GND");
    while (1) delay(10);
  }
  ...
  display.println("Discharge Mode");
  display.display();
  Serial.println("Discharge Mode Started");
  delay(2000);
}
...
void loop() {
  ...
  float remaining_time_h = (load_current_mA > 0) ? capacity_mAh / load_current_mA : 0.0;
  
  Serial.print("Battery V: "); Serial.print(battery_V, 2);
